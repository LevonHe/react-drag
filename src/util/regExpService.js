...
// url：以http开头
export const HttpUrlReg = /^https?:\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\\/=\\?%\-&_~`@[\]\\':+!]*([^<>\\"\\"])*$/;
// 版本号：格式如a.b.c，a、b、c由最大4位数字组合
export const VersionReg = /^[0-9]{1,4}\.\d{1,4}\.\d{1,4}$/; // 版本号：格式如a.b.c，a、b、c由最大4位数字组合
// 标识符：1～32位，字符：英文、"_-"、数字、允许中间有空格
export const IdentifierReg = /^[A-Za-z0-9_-](([A-Za-z0-9_-\s]{0,30}[A-Za-z0-9_-])|([A-Za-z0-9_-]{0,31}))$/;
// 通用：允许中间有空格
export const TrimReg = /^\S(([\s\S]{0,253}\S)|([\s\S]{0,254}))$/;
// 用户名：8～32位，字符：由8~32位英文字母、数字、@、点、下划线、中划线(除回车、TAB外)组成
export const UserNameReg = /^\S((.{0,30}\S)|(\S{0,31}))$/;
...
// 名称：1～255位，字符：由1~255位英文字母、数字、@、点、下划线、中划线(除回车、TAB外)组成
export const Name255Reg = /^\S((.{0,253}\S)|(\S{0,254}))$/;
...
// 正整数
export const PositiveInteger = /^\d{1,6}$/;
